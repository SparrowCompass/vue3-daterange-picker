...
export default {
    ...
    methods: {
        ...
        changeLeftMonth(value) {
            ...
            if (
                ...
                this.$dateUtil.yearMonth(this.monthDate) >= this.$dateUtil.yearMonth(this.nextMonthDate)
            ) {
        ...
        changeRightMonth(value) {
            ...
            if (
                this.linkedCalendars ||
                this.$dateUtil.yearMonth(this.nextMonthDate) <= this.$dateUtil.yearMonth(this.monthDate)
            ) {
                this.monthDate = this.$dateUtil.validateDateRange(
    ...
    computed: {
        ...
        isDirty() {
            ...
            return (
                !this.isClear &&
                (this.start.getTime() !== origStart.getTime() || this.end.getTime() !== origEnd.getTime())
            );
        },
    },
    watch: {
        ...
        'modelValue.startDate'(value) {
            ...
            this.start =
                !!value && !this.isClear && this.$dateUtil.isValidDate(new Date(value)) ? new Date(value) : null;
            if (this.isClear) {
                this.start = null;
                this.end = null;
            } else {
                    ...
                    this.$nextTick(() => {
                        ...
                        if (!this.alwaysShowCalendars && this.ranges) {
                            this.showCustomRangeCalendars = !Object.keys(this.ranges).find(
                                (key) =>
                                    this.$dateUtil.isSame(this.start, this.ranges[key][0], 'date') &&
